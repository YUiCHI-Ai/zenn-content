...
:::message
この記事は「HOW（使い方・設定方法）」にフォーカスしている。「なぜNode.jsはnpmだけを同梱してきたのか」「Corepack非バンドル化の技術的背景は何か」といったWHY（設計思想・歴史的経緯）については、本記事の範囲外とし、末尾の参考リンクを参照してほしい。
:::
...
| Node.js 25以降の影響 | 非バンドル化の影響あり | 影響なし |
...
Node.jsコアチームは、**Node.js 25以降でCorepackをNode.jsにバンドルしない**方針を発表している。これは計画・提案段階であり、Node.js 25のリリース時期に向けて議論が進んでいる。

廃止ではなく「非バンドル化」であることに注意してほしい。Corepack自体はnpmパッケージとして存続し、`npm install -g corepack` で引き続き利用できる。

### 非バンドル化の理由

Node.jsの公式ディスカッション（GitHub issue）で挙げられている主な理由は以下だ。
...
```yaml
# Node.js 25以降で失敗するワークフロー
- uses: actions/setup-node@v4
  with:
    node-version: '25'
- run: corepack enable  # ← コマンドが存在しない
```
...
公式Node.jsのDockerイメージ（`node:25`以降）にもCorepackが含まれなくなる。
...
Node.js 25はまだリリースされていないが、今から以下を準備しておくと移行がスムーズだ。

1. **CIワークフローを確認**: `corepack enable` に依存している箇所を洗い出す
2. **専用アクションへの移行を検討**: `pnpm/action-setup` などに切り替える
3. **Dockerfileを確認**: Corepackに依存している行を特定する
4. **`packageManager` フィールドは維持する**: Corepackがなくなっても、`packageManager` フィールド自体はNode.jsエコシステムの標準として残る可能性が高い。`pnpm/action-setup` などのツールがこのフィールドを読み取る
...
- **Node.js 25以降への移行が近い**プロジェクトで、依存を増やしたくない場合
...
この構成なら、ローカルではCorepackの恩恵を受けつつ、CIではCorepackに依存しない安定した環境を構築できる。Node.js 25以降でCorepackが非バンドル化されても、CIへの影響はない。
